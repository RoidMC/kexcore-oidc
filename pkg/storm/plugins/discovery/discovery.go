...
func New(endpoints Endpoints, keyStore storm.KeyStore) *Plugin {
	return &Plugin{endpoints: endpoints, keyStore: keyStore}
}
...
func (p *Plugin) Contribute(ctx context.Context) map[string]any {
	...
	// Determine signing algorithms from KeyStore.
	// This includes GM/T algorithms (SGD_SM3_SM2, SGD_SM3_SM9) when available.
	...
	if p.keyStore != nil {
		if algs, err := p.keyStore.SignatureAlgorithms(ctx); err == nil && len(algs) > 0 {
			...
			// Auth signing algs include all signing algs
	...
	cfg := map[string]any{
		...
		"backchannel_logout_uri":                           makeURL(issuer, p.endpoints.BackChannelLogout),
		"frontchannel_logout_uri":                          makeURL(issuer, p.endpoints.FrontChannelLogout),
		...
		"response_types_supported":                         []string{"code", "id_token", "id_token token", "code id_token", "code token", "code id_token token"},
		"grant_types_supported":                            []string{"authorization_code", "implicit", "password", "client_credentials", "refresh_token", "urn:ietf:params:oauth:grant-type:jwt-bearer", "urn:ietf:params:oauth:grant-type:token-exchange", "urn:ietf:params:oauth:grant-type:device_code"},
