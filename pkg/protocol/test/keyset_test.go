...
func TestFindKey(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args{
				keyID:       "",
				use:         protocol.KeyUseSignature,
				expectedAlg: "RS256",
				keys:        nil,
			},
			res{
				err: protocol.ErrKeyNone,
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := protocol.FindMatchingKey(tt.args.keyID, tt.args.use, tt.args.expectedAlg, tt.args.keys...)
