...
// Package testutil helps setting up required data for testing,
...
func (KeySet) VerifySignature(ctx context.Context, rawToken []byte) (payload []byte, err error) {
	...
	return jws.Verify(rawToken, jws.WithKey(sigAlg, webPubKey))
}
...
func init() {
	...
	webPubKey, err = webKeyPair.PublicKey()
	if err != nil {
		panic(err)
	}
}
...
func (JWTProfileKeyStorage) GetKeyByIDAndClientID(ctx context.Context, keyID string, clientID string) (jwk.Key, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}

	pubKey, err := webKeyPair.PublicKey()
	if err != nil {
		return nil, err
	}
...
func signEncodeTokenClaims(claims any) string {
	...
	signed, err := jws.Sign(payload, jws.WithKey(jwa.RS256(), webKeyPair))
	if err != nil {
		panic(err)
	}
