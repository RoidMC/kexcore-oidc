...
class Conformance(object):
    ...
    async def upload_image(self, module_id, image_bytes, filename="screenshot.png"):
        """Upload an image to a test module that is waiting for a screenshot."""
        api_url = '{0}api/log/{1}/image'.format(self.api_url_base, module_id)
        files = {'image': (filename, image_bytes, 'image/png')}
        # Use a separate client for multipart upload (no content-type header)
        with httpx.Client(verify=self.verify_ssl) as client:
            headers = {k: v for k, v in self.httpclient.headers.items() if k.lower() != 'content-type'}
            response = client.post(api_url, files=files, headers=headers)
        if response.status_code not in (200, 201):
            raise Exception("upload_image failed - HTTP {:d} {}".format(response.status_code, response.content))
        return response
    ...
    async def stop_module(self, module_id):
        api_url = '{0}api/runner/{1}'.format(self.api_url_base, module_id)
        response = self.httpclient.delete(api_url)
        return response

    async def restart_module(self, module_id):
        api_url = '{0}api/runner/{1}'.format(self.api_url_base, module_id)
        response = self.httpclient.put(api_url)
        return response
